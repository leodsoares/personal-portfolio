...
function SkillCard({ skill, index }: { skill: Skill; index: number }) {
  return (
    <motion.div
      ...
      className="group bg-gray-800/60 border border-gray-700 hover:border-indigo-500/50 rounded-xl p-4 flex flex-col items-center gap-2.5 transition-colors hover:bg-gray-800 cursor-default"
    >
      {skill.icon ? (
      ...
      <span className="text-xs text-gray-400 group-hover:text-gray-200 transition-colors font-medium text-center">
        {skill.name}
      </span>
    </motion.div>
  );
    ...
    <div>
      ...
      <motion.div
        ...
        className={`rounded-2xl overflow-hidden cursor-pointer transition-colors duration-300 ${
          expanded ? 'bg-gray-900 border border-indigo-500/30' : 'bg-gray-900 border border-gray-800'
        }`}
...
const proficiencyColor: Record<string, string> = {
  ...
  Fluent: 'text-indigo-400',
  Advanced: 'text-sky-400',
  Intermediate: 'text-amber-400',
  'Langue maternelle': 'text-green-400',
  Courant: 'text-indigo-400',
  Avancé: 'text-sky-400',
  Intermédiaire: 'text-amber-400',
  Nativo: 'text-green-400',
  Fluente: 'text-indigo-400',
  Avançado: 'text-sky-400',
...
function LanguageCard({
  ...
  const color = proficiencyColor[lang.proficiency] ?? 'text-gray-400';
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={inView ? { opacity: 1, y: 0 } : {}}
